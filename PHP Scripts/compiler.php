<?php
	...
	if(empty($error)){
		...
		if(!empty($input)){
			shell_exec("timeout 1s ./".$executable." < ".$filename_in." 2> ".$filename_error);
			$error= file_get_contents($filename_error);
			if (empty($error)) {
				$output=shell_exec("timeout 1s ./".$executable." < ".$filename_in);
			}
		}
		else{
			shell_exec("timeout 1s ./".$executable." 2> ".$filename_error);
			$error = file_get_contents($filename_error);
			if (empty($error)){
				$output=shell_exec("timeout 1s ./".$executable);
			}
		}
	}
	if(isset($output))
